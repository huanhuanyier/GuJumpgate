      ...
      if (manualVerificationUrl) {
        ...
        if (!code) {
          ...
          if (noCodeError.hostedCheckoutResendImmediately) {
            const responsePreview = noCodeError.hostedCheckoutResponsePreview
              || getHostedCheckoutNoCodeResponsePreview(payload);
            noCodeError.hostedCheckoutResendImmediately = false;
            noCodeError.message = responsePreview
              ? `hosted checkout 验证码接口暂未返回有效验证码，响应预览：${responsePreview}`
          ...
          if (!noCodeError.hostedCheckoutResendImmediately) {
            try {
              ...
              await addLog(`步骤 6：手动取码浏览器标签页兜底未成功：${fallbackError?.message || fallbackError}`, 'warn');
            }
          }
          throw noCodeError;
        }
          ...
          if (isHostedCheckoutNonFreeTrialFailure(error)) {
            ...
            await addLog(
              shouldRetryStep7Only
                ? `${stopReason} 先手机号注册 OAuth 将保留当前注册流程，直接回到第 7 步重新创建 Checkout。`
